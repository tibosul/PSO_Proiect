...
bool ip_is_network_address(struct in_addr ip, struct in_addr network,
...
bool ip_is_broadcast_address(struct in_addr ip, struct in_addr network,
                             struct in_addr netmask) {
  uint32_t ip_val = ntohl(ip.s_addr);
  uint32_t net_val = ntohl(network.s_addr);
  uint32_t mask_val = ntohl(netmask.s_addr);
...
struct ip_pool_entry_t *ip_pool_find_entry(struct ip_pool_t *pool,
                                           struct in_addr ip) {
  if (!pool)
    return NULL;
...
bool ip_ping_check(struct in_addr ip, uint32_t timeout_ms) {
  ...
  if (sendto(sockfd, packet, sizeof(icmp_hdr), 0, (struct sockaddr *)&addr,
             sizeof(addr)) < 0) {
    close(sockfd);
    return false;
  }
...
int ip_pool_init(struct ip_pool_t *pool, struct dhcp_subnet_t *subnet,
  ...
  for (uint32_t ip_val = start_ip;
    ...
    if (ip_is_network_address(entry->ip_address, subnet->network,
                              subnet->netmask)) {
      entry->state = IP_STATE_EXCLUDED;
    } else if (ip_is_broadcast_address(entry->ip_address, subnet->network,
                                       subnet->netmask)) {
      entry->state = IP_STATE_EXCLUDED;
...
int ip_pool_reserve_ip(struct ip_pool_t *pool, struct in_addr ip,
                       const uint8_t mac[6]) {
  if (!pool || !mac)
    return -1;
...
struct ip_allocation_result_t ip_pool_allocate(struct ip_pool_t *pool,
                                               const uint8_t mac[6],
  ...
  if (requested_ip.s_addr != 0) {
    if (ip_pool_is_in_range(pool, requested_ip) &&
        ip_pool_is_available(pool, requested_ip)) {
      ...
      if (config->global.ping_check) {
        if (ip_ping_check(requested_ip, config->global.ping_timeout * 1000)) {
          ...
          struct ip_pool_entry_t *entry =
              ip_pool_find_entry(pool, requested_ip);
          if (entry) {
            if (entry->state == IP_STATE_AVAILABLE)
              pool->available_count--;
            entry->state = IP_STATE_ALLOCATED;
  ...
  for (uint32_t i = 0; i < pool->pool_size; i++) {
    ...
    if (entry->state == IP_STATE_AVAILABLE) {
      // Ping check if enabled
      if (config->global.ping_check) {
        if (ip_ping_check(entry->ip_address,
                          config->global.ping_timeout * 1000)) {
...
int ip_pool_update_from_lease(struct ip_pool_t *pool,
...
int ip_pool_sync_with_leases(struct ip_pool_t *pool,
                             struct lease_database_t *lease_db) {
  if (!pool || !lease_db)
    return -1;
...
struct dhcp_lease_t *ip_pool_allocate_and_create_lease(
    struct ip_pool_t *pool, struct lease_database_t *lease_db,
  ...
  struct ip_allocation_result_t result =
      ip_pool_allocate(pool, mac, requested_ip, config);
  ...
  struct dhcp_lease_t *existing_lease =
      lease_db_find_by_ip(lease_db, result.ip_address);
  ...
  if (existing_lease) {
    ...
    if (lease_db_renew_lease(lease_db, result.ip_address, lease_time) == 0) {
      ...
      struct ip_pool_entry_t *entry =
          ip_pool_find_entry(pool, result.ip_address);
      if (entry) {
    ...
    // Create new lease
    struct dhcp_lease_t *new_lease =
        lease_db_add_lease(lease_db, result.ip_address, mac, lease_time);
...
void ip_pool_print_stats(const struct ip_pool_t *pool) {
  ...
  printf("Utilization: %.1f%%\n",
         pool->pool_size > 0 ? (pool->allocated_count * 100.0 / pool->pool_size)
                             : 0.0);
}
