...
void packet_processor(void *arg) {
  ...
  struct dhcp_subnet_t *subnet =
      &g_server.config.subnets[0]; // Naive: use first subnet
  ...
  case DHCP_DISCOVER: {
    ...
    if (!lease) {
      ...
      uint8_t *req_ip_opt =
          dhcp_message_get_option(req, DHCP_OPT_REQUESTED_IP, NULL);
      if (req_ip_opt)
...
int main(int argc, char *argv[]) {
  ...
  while (g_running) {
    ...
    task->len =
        recvfrom(g_server.sockfd, &task->packet, sizeof(struct dhcp_packet), 0,
