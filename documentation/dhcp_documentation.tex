...
\IEEEauthorblockA{\textit{Departamentul de Calculatoare} \\
\textit{Universitatea Politehnica București}\\
București, România \\
email@student.upb.ro}
}
...
\begin{enumerate}
    ...
    \item Dezvoltarea unui server DHCP v6 cu suport pentru stateful address assignment (RFC 8415).
    ...
    \item Suport pentru multiple configurații de rețea: loopback, LAN, VLAN și cross-machine.
...
\begin{itemize}
    ...
    \item \textbf{Prefix Delegation}: DHCPv6 suportă alocarea de prefixe întregi (/48, /56, /64) pentru routere.
\end{itemize}
...
192.168.1.100 aa:bb:cc:dd:ee:ff 
  1705772400 7200 *
10.0.0.50 11:22:33:44:55:66 
  1705775600 3600 host1
\end{verbatim}

Format: \texttt{IP MAC\_ADDR START\_TIME DURATION HOSTNAME}

\subsection{Aspecte Tehnice Relevante}

\textbf{Gestionarea concurenței}:
\begin{itemize}
    \item Thread pool cu dimensiune configurabilă (implicit 4 threads)
...
\textbf{Fragmenta cod semnificativă - Procesare DISCOVER}:
...
\begin{itemize}
    ...
    \item \textit{Setup}: VirtualBox cu bridged networking
...
\begin{itemize}
    ...
    \item Scenarii de testare: 6/6 passed
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{IPv6 Prefix Delegation}: Implementare completă RFC 8415
    \item \textbf{Statistici și analytics}: Dashboard pentru monitorizare
...
Mulțumim coordonatorului de proiect pentru îndrumarea și suportul oferit pe parcursul dezvoltării acestui sistem. De asemenea, mulțumim comunității open-source pentru documentațiile și tooling-ul care au facilitat implementarea.
